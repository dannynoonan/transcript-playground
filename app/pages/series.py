...
def layout(show_key: str) -> html.Div:
    ...
    series_summary, episodes_by_season = generate_series_summary(show_key)
    all_season_episode_data = generate_all_season_episode_data(show_key, episodes_by_season, series_summary)
    season_accordion_items = cmp.generate_season_episodes_accordion_items(all_season_episode_data, speaker_color_map)

    # topic listing data
    universal_genres_parent_topics = get_parent_topics_for_grouping('universalGenres')
...
######################################## SERIES PAGELOAD READ FUNCTIONS ##########################################
