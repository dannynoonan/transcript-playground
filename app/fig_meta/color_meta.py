...
def flatten_speaker_colors(show_key: str, to_rgb: bool = False) -> dict:
    ...
    for s, d in show_metadata[show_key]['regular_cast'].items():
        if to_rgb:
            ...
            speaker_color_map[s] = f'rgb{rgb}'.replace(' ', '')
        else:
            speaker_color_map[s] = d['color']
...
def convert_speaker_color_discrete_map_to_rgb(speaker_color_map: dict) -> dict:
    '''
    NOTE a lot of this format conversion is due to happenstance, with different formats being needed at different points
    TODO come back and methodically organize color map building, preferably using one and only one format for all scenarios
    '''
    speaker_color_map_rgb = {}
    for spkr, color in speaker_color_map.items():
        rgb = ImageColor.getcolor(color, 'RGB')
        speaker_color_map_rgb[spkr] = f'rgb{rgb}'.replace(' ', '')

    return speaker_color_map_rgb
