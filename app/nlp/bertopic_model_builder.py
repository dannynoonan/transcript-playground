...
def generate_bertopic_models(show_key: str, bert_text_inputs: list, config: dict, sources_df: pd.DataFrame) -> dict|None:
    ...
    print(f'logs={logs}')
    f = open(f'{BERTOPIC_LOGS_DIR}/log_{show_key}', 'a')  # append mode
    f.write('-----------------------------------------------------------------------------------------\n')
    ...
    os.mkdir(f'{BERTOPIC_MODELS_DIR}/{show_key}/{model_id}')
    os.mkdir(f'{BERTOPIC_MODELS_DIR}/{show_key}/{model_id}/mmr')
    os.mkdir(f'{BERTOPIC_MODELS_DIR}/{show_key}/{model_id}/kb')
    os.mkdir(f'{BERTOPIC_MODELS_DIR}/{show_key}/{model_id}/openai')
    # save models
    se_model = f'sentence-transformers/{sentence_transformer_lm}'
    mmr_bertopic_model.save(f'{BERTOPIC_MODELS_DIR}/{show_key}/{model_id}/mmr', serialization='safetensors', save_ctfidf=True, save_embedding_model=se_model)
    kb_bertopic_model.save(f'{BERTOPIC_MODELS_DIR}/{show_key}/{model_id}/kb', serialization='safetensors', save_ctfidf=True, save_embedding_model=se_model)
    openai_bertopic_model.save(f'{BERTOPIC_MODELS_DIR}/{show_key}/{model_id}/openai', serialization='safetensors', save_ctfidf=True, save_embedding_model=se_model)
    # save bertopic_docs_df
    bertopic_docs_file_name = f'{BERTOPIC_DATA_DIR}/{show_key}/{model_id}.csv'
...
def get_model_response(messages, prompt):
    ...
    # NOTE untested, but this is how sentiment analysis requests were formatted (in Sep 2024 vs May 2024)
