...
def publish_animation(show_key: str, fig_type: str, span_granularity: str = None, season: str = None):
    ...
    if fig_type == 'speaker_frequency_bar':
        ...
        if season:
            fig = pb.build_speaker_frequency_bar(show_key, df, span_granularity, True, int(season), animate=True)
            output_path = f'static/animations/{show_key}/{fig_type}_{show_key}_S{season}_{span_granularity}.html'
            ...
            fig = pb.build_speaker_frequency_bar(show_key, df, span_granularity, False, None, animate=True)
            output_path = f'static/animations/{show_key}/{fig_type}_{show_key}_SERIES_{span_granularity}.html'
            fig.write_html(output_path, auto_play=False)
            print(f'Successfully generated and saved animation html file to output_path={output_path}')
