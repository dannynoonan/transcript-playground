...
def main():
    ...
    for doc_id in episode_doc_ids:
        ...
        try:
            esw.populate_episode_topics(ShowKey(show_key), episode_key, topic_grouping, model_vendor, model_version, user_dependency)
    ...
    for e_key, t_keys_to_scores in ekey_tkey_scores.items():
        ...
        if topic_grouping in ['universalGenres'] and model_vendor == 'openai' and model_version == '3small':
            ...
            esw.populate_episode_tfidf_topics(ShowKey(show_key), e_key, topic_grouping, simple_episode_topics, user_dependency)
